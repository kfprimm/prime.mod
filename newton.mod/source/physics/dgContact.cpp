...
void dgContact::JointAccelerationsSimd(const dgJointAccelerationDecriptor& params)
{
...
//	simd_type invTimeStep;
	...
	for (dgInt32 k = 0; k < params.m_rowsCount; k ++) {
		if (!params.m_accelIsMotor[k]) {
			...
			simd_type relVeloc = simd_mul_v (              (simd_type&)Jt[k].m_jacobian_IM0.m_linear, bodyVeloc0); 
			relVeloc = simd_mul_add_v (relVeloc, (simd_type&)Jt[k].m_jacobian_IM0.m_angular, bodyOmega0); 
			relVeloc = simd_mul_add_v (relVeloc, (simd_type&)Jt[k].m_jacobian_IM1.m_linear, bodyVeloc1); 
			relVeloc = simd_mul_add_v (relVeloc, (simd_type&)Jt[k].m_jacobian_IM1.m_angular, bodyOmega1); 
			...
			if (params.m_normalForceIndex[k] < 0) {
				...
				//			}
				simd_type restitution = simd_sub_s (simd_and_v (simd_set1 (params.m_restitution[k]), simd_cmplt_s (relVeloc, zero)), negOne);
				...
				simd_type penetrationVeloc = simd_and_v (simd_mul_s (penetration, simd_load_s (params.m_penetrationStiffness[k])), penetrationMask);
			...
			simd_store_s (simd_mul_sub_s (relAccel, relVeloc, invTimeStep), &params.m_coordenateAccel[k]);
		}
	}
#endif
}
...
void dgCollidingPairCollector::FlushChache (dgThreadPairCache* const pairChache)
{
	...
	while ((m_count + pairChache->m_count) > m_maxSize) {
		void* newBuffer;
		newBuffer = world->m_allocator->Malloc (2 * world->m_pairMemoryBufferSizeInBytes) ;
	...
	memcpy (&m_pairs[m_count], pairChache->m_chacheBuffer, sizeof (dgPair) * pairChache->m_count);
