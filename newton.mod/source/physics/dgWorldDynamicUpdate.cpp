...
static inline dgInt32 CompareIslands (const dgIsland* const islandA, const dgIsland* const islandB, void* notUsed)
{
	dgInt32 countA;
	dgInt32 countB;
...
void dgWorldDynamicUpdate::UpdateDynamics(dgWorld* const world, dgInt32 archModel, dgFloat32 timestep)
{
...
void dgSolverWorlkerThreads::ThreadExecute()
{
	...
	dgContactPoint* const contactBuffer = (dgContactPoint*) m_world->m_contactBuffers[m_threadIndex];
...
void dgWorldDynamicUpdate::ReallocBodyMemory (dgInt32 bodyCount)
{
	...
	if (bodyCount) {
		...
		memory = m_world->GetAllocator()->MallocLow (m_world->m_bodiesMemorySizeInBytes); 
...
void dgWorldDynamicUpdate::ReallocIslandMemory (dgInt32 islandCount)
{
	...
	if (islandCount) {
		...
		memory = m_world->GetAllocator()->MallocLow (m_world->m_islandMemorySizeInBytes); 
...
void dgWorldDynamicUpdate::ReallocJointsMemory (dgInt32 count)
{
	...
	if (count) {
		...
		memory = m_world->GetAllocator()->MallocLow (m_world->m_jointsMemorySizeInBytes); 
...
void dgWorldDynamicUpdate::ReallocJacobiansMemory (dgInt32 count, dgInt32 threadIndex)
{
	void *memory;
	dgInt32 stride;
	dgInt32 newCount;
	...
	if (count) {
		m_world->m_jacobiansMemorySizeInBytes[threadIndex] = m_world->m_jacobiansMemorySizeInBytes[threadIndex] * 2;
		memory = m_world->GetAllocator()->MallocLow (m_world->m_jacobiansMemorySizeInBytes[threadIndex] + 64); 
		...
		upperBoundFrictionCoefficent = (dgFloat32*) &lowerBoundFrictionCoefficent[newCount];
	...
	m_solverMemory[threadIndex].m_lowerBoundFrictionCoefficent = lowerBoundFrictionCoefficent; 
	m_solverMemory[threadIndex].m_upperBoundFrictionCoefficent = upperBoundFrictionCoefficent; 
...
void dgWorldDynamicUpdate::ReallocIntenalForcesMemory (dgInt32 count, dgInt32 threadIndex)
{
	dgInt32 stride;
	dgInt32 newCount;
	void *memory;
	...
	stride = sizeof (dgJacobian) + 
		sizeof (dgJacobian) + 
		sizeof (dgInt32) +
		0;

	if (count) {
		m_world->m_internalForcesMemorySizeInBytes[threadIndex] = m_world->m_internalForcesMemorySizeInBytes[threadIndex] * 2;
		memory = m_world->GetAllocator()->MallocLow (m_world->m_internalForcesMemorySizeInBytes[threadIndex]); 
		...
		m_world->GetAllocator()->FreeLow (m_world->m_internalForcesMemory[threadIndex]);
	} else {
	...
	_ASSERTE ((((dgInt8 *)&treadLocks[newCount - 1]) - ((dgInt8 *)memory)) < m_world->m_internalForcesMemorySizeInBytes[threadIndex]);
...
void dgWorldDynamicUpdate::BuildIsland (dgQueue<dgBody*>& queue, dgInt32 jointCount, dgInt32 hasUnilateralJoints, dgInt32 isContinueCollisionIsland)
{
	dgInt32 bodyCount = 1;
...
void dgWorldDynamicUpdate::SpanningTree (dgBody* const body)
{
	...
	dgQueue<dgBody*> queue ((dgBody**) m_world->m_pairMemoryBuffer, (dgInt32 (m_world->m_pairMemoryBufferSizeInBytes>>1))/dgInt32 (sizeof (void*)));
...
//isInEquilibrium = 0;
	...
	if (isInEquilibrium | !isInWorld) {
		...
		BuildIsland (queue, jointCount, hasUnilateralJoints, isContinueCollisionIsland);
	}
}


void dgWorldDynamicUpdate::IntegrateArray (
	const dgBodyInfo* bodyArray, 
	dgInt32 count, 
	dgFloat32 accelTolerance, 
	dgFloat32 timestep,
	dgInt32 threadIndex,
	bool update) const
{
		...
		if (isAutoSleep) {
			if (stackSleeping) {
				...
				if ((maxAccel > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxAccel) ||
					(maxAlpha > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxAlpha) ||
					(maxSpeed > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxVeloc) ||
					(maxOmega > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxOmega)) {
						for (dgInt32 i = 0; i < count; i ++) {
							dgBody* const body = bodyArray[i].m_body;
...
dgInt32 dgWorldDynamicUpdate::GetJacobialDerivatives (
	...
	for (dgInt32 j = 0; j < jointCount; j ++) {
		...
		if (constraint->m_isUnilateral ^ bitMode) {
			...
			for (dgInt32 i = 0; i < dof; i ++) {
				...
				solverMemory.m_accelIsMotor[rowCount] = dgInt32 (constraintParams.m_isMotor[i]);
...
void dgWorldDynamicUpdate::BuildJacobianMatrixSimd (const dgIsland& island, dgInt32 threadIndex, dgFloat32 timestep)
{
#ifdef DG_BUILD_SIMD_CODE
//	dgInt32 threads;
//	dgInt32 rowCount;
...
//	dgInt32 threads = m_world->m_numberOfTheads;
	...
	for (dgInt32 j = 0; j < jointCount; j ++) {
		dgConstraint* const constraint = constraintArray[j].m_joint;
		dgInt32 dof = dgInt32 ((constraint->m_maxDOF & (DG_SIMD_WORD_SIZE - 1)) ? ((constraint->m_maxDOF & (-DG_SIMD_WORD_SIZE)) + DG_SIMD_WORD_SIZE) : constraint->m_maxDOF);
		maxRowCount += dof;
	}
	...
	if (island.m_hasUnilateralJoints) {
		rowCount = GetJacobialDerivatives (island, threadIndex, false, rowCount, timestep);
	}
	rowCount = GetJacobialDerivatives (island, threadIndex, true, rowCount, timestep);
...
//		dgBody* body1;
		...
		for (dgInt32 i = 0; i < count; i ++) {
			...
			simd_type tmpDiag = simd_mul_v ((simd_type&)JMinv[index].m_jacobian_IM0.m_linear, (simd_type&)Jt[index].m_jacobian_IM0.m_linear);
			...
			simd_type tmpAccel = simd_mul_v ((simd_type&)JMinv[index].m_jacobian_IM0.m_linear, (simd_type&)body0->m_accel);
...
void dgWorldDynamicUpdate::BuildJacobianMatrix (const dgIsland& island, dgInt32 threadIndex, dgFloat32 timestep)
{
	dgVector zeroVector (dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(0.0f));
...
void dgJacobianMemory::CalculateReactionsForcesSimd (dgInt32 solverMode, dgFloat32 tolerance) const
{
#ifdef DG_BUILD_SIMD_CODE
...
void dgJacobianMemory::CalculateReactionsForces (dgInt32 solverMode, dgFloat32 tolerance) const
{
	if (m_jointCount == 0) {
...
void dgJacobianMemory::ApplyExternalForcesAndAccelerationSimd (dgFloat32 tolerance) const
{
#ifdef DG_BUILD_SIMD_CODE
...
//			maxForce = simd_max_s (maxForce, simd_and_v (val, absMask));
			...
			y0_linear = simd_mul_add_v (y0_linear, (simd_type&)Jt[index].m_jacobian_IM0.m_linear, val);
			y0_angular = simd_mul_add_v (y0_angular, (simd_type&)Jt[index].m_jacobian_IM0.m_angular, val);
			y1_linear = simd_mul_add_v (y1_linear, (simd_type&)Jt[index].m_jacobian_IM1.m_linear, val);
			y1_angular = simd_mul_add_v (y1_angular, (simd_type&)Jt[index].m_jacobian_IM1.m_angular, val);
		}
...
void dgJacobianMemory::CalculateSimpleBodyReactionsForcesSimd (dgFloat32 maxAccNorm) const
{
#ifdef DG_BUILD_SIMD_CODE
...
//	dgBody* const body1 = bodyArray[m1].m_body;
	...
	for (dgInt32 i = 0; i < roundCount; i += DG_SIMD_WORD_SIZE) {
		...
		simd_type normalForce = simd_move_lh_v (simd_pack_lo_v (simd_load_s(force[normalForceIndex[i + 0]]), 
		...
		(simd_type&)lowBound[i] = simd_mul_v (normalForce, (simd_type&)lowerFriction[i]);
		(simd_type&)highBound[i] = simd_mul_v (normalForce,  (simd_type&)upperFriction[i]);
		...
		simd_type test = simd_or_v (simd_cmplt_v ((simd_type&)force[i], (simd_type&)lowBound[i]), simd_cmpgt_v ((simd_type&)force[i], (simd_type&)highBound[i]));
		...
		(simd_type&) force[i] = simd_min_v ((simd_type&) highBound[i], simd_max_v((simd_type&) force[i], (simd_type&)lowBound[i]));
	}

	tmp2 = simd_add_v (tmp2, simd_move_hl_v(tmp2, tmp2));
	dgInt32 maxPasses = simd_store_is (simd_add_s (tmp2, simd_permut_v (tmp2, tmp2, PURMUT_MASK (0, 0, 0, 1))));
	...
	for (dgInt32 i = 0; i < count; i ++) {
		...
		y0_linear  = simd_mul_add_v (y0_linear, (simd_type&) Jt[i].m_jacobian_IM0.m_linear, tmp1);
		y0_angular = simd_mul_add_v (y0_angular,(simd_type&) Jt[i].m_jacobian_IM0.m_angular, tmp1);
		y1_linear  = simd_mul_add_v (y1_linear, (simd_type&) Jt[i].m_jacobian_IM1.m_linear, tmp1);
		y1_angular = simd_mul_add_v (y1_angular,(simd_type&) Jt[i].m_jacobian_IM1.m_angular, tmp1);
	}
	...
	for (dgInt32 i = 0; i < count; i ++) {
		...
		simd_type tmp2 = simd_mul_v (          (simd_type&)JMinv[i].m_jacobian_IM0.m_linear, y0_linear);
		tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[i].m_jacobian_IM0.m_angular, y0_angular);
		tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[i].m_jacobian_IM1.m_linear, y1_linear);
		tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[i].m_jacobian_IM1.m_angular, y1_angular);
		...
		tmp2 = simd_sub_s(simd_load_s(coordenateAccel[i]), simd_mul_add_s(tmp2, simd_load_s(force[i]), simd_load_s(diagDamp[i])));
		simd_store_s (tmp2, &accel[i]);

		//m_deltaForce[i] = m_accel[i] * m_invDJMinvJt[i] * activeRow[i];
		simd_type tmp3 = simd_mul_s(tmp2, simd_mul_s(simd_load_s (invDJMinvJt[i]), simd_load_s (activeRow[i])));
		...
		tmp1 = simd_max_s(tmp1, simd_and_v(simd_mul_v (tmp2, simd_load_s (activeRow[i])), signMask));
	}
...
//		dgFloat32 clampedForceIndexValue;
		...
		for (dgInt32 k = 0; k < count; k ++) {
			...
			simd_type tmp2 = simd_mul_v (          (simd_type&)JMinv[k].m_jacobian_IM0.m_linear, y0_linear);
			tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[k].m_jacobian_IM0.m_angular, y0_angular);
			tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[k].m_jacobian_IM1.m_linear, y1_linear);
			tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[k].m_jacobian_IM1.m_angular, y1_angular);
...
//			simd_type posDeltaForceTest;
			...
			simd_type val = simd_mul_add_v ((simd_type&)force[k], tmp0, (simd_type&)deltaForce[k]);
			...
			simd_type negDeltaForceTest = simd_cmplt_v ((simd_type&)deltaForce[k], tol_neg_1eNeg16);

			//			posDeltaForceTest = deltaForce[k] > dgFloat32 ( 1.0e-16f);
			simd_type posDeltaForceTest = simd_cmpgt_v ((simd_type&)deltaForce[k], tol_pos_1eNeg16);
			...
			simd_type num = simd_or_v (simd_and_v ((simd_type&)lowBound[k], negTest), simd_and_v ((simd_type&)highBound[k], posTest)); 
			num = simd_or_v (simd_and_v (num, test), simd_andnot_v ((simd_type&)force[k], test)); 

			//			den = test ? deltaForce[k] : dgFloat32 (1.0f);
			simd_type den = simd_or_v (simd_and_v ((simd_type&)deltaForce[k], test), simd_andnot_v (one, test)); 
			...
			tmp0 = simd_or_v (simd_div_v (simd_sub_v (num, (simd_type&)force[k]), den), simd_andnot_v (tmp0, test)); 
		...
		minClampIndex = simd_or_v (simd_and_v (minClampIndex, tmp3), simd_andnot_v (simd_move_hl_v(minClampIndex, minClampIndex), tmp3));
		campedIndexValue = simd_or_v (simd_and_v (campedIndexValue, tmp3), simd_andnot_v (simd_move_hl_v(campedIndexValue, campedIndexValue), tmp3));

		tmp2 = simd_permut_v (tmp0, tmp0, PURMUT_MASK (0, 0, 0, 1));
		tmp3 = simd_cmplt_s (tmp0, tmp2);
		tmp0 = simd_min_s (tmp0, tmp2);
		minClampIndex = simd_or_v (simd_and_v (minClampIndex, tmp3), simd_andnot_v (simd_permut_v (minClampIndex, minClampIndex, PURMUT_MASK (0, 0, 0, 1)), tmp3));
		campedIndexValue = simd_or_v (simd_and_v (campedIndexValue, tmp3), simd_andnot_v (simd_permut_v (campedIndexValue, campedIndexValue, PURMUT_MASK (0, 0, 0, 1)), tmp3));
		...
		if (ak == dgFloat32 (0.0f) && (clampedForceIndex != -1)) {
			...
			for (dgInt32 k = 0; k < roundCount; k += DG_SIMD_WORD_SIZE) {
				...
				simd_type val_k = simd_and_v (simd_sub_v ((simd_type&)lowBound[k], force_k), signMask);

				//test0 = (val < dgFloat32 (1.0e-5f)) & (accel[k] < dgFloat32 (0.0f));
				simd_type test_0 = simd_and_v (simd_cmplt_v (val_k, tol_pos_1eNeg5), simd_cmplt_v (accel_k, zero)); 

				//val = dgAbsf (highBound[k] - force[k]);
				val_k = simd_and_v (simd_sub_v ((simd_type&)highBound[k], force_k), signMask);

				//test1 = (val < dgFloat32 (1.0e-5f)) & (accel[k] > dgFloat32 (0.0f));
				simd_type test_1 = simd_and_v (simd_cmplt_v (val_k, tol_pos_1eNeg5), simd_cmpgt_v (accel_k, zero)); 
				...
				(simd_type&) force[k] = simd_min_v ((simd_type&) highBound[k], simd_max_v((simd_type&) force[k], (simd_type&)lowBound[k]));

				//activeRow[k] *= (test0 | test1) ? dgFloat32 (0.0f) : dgFloat32 (1.0f);
				//test_2 = simd_or_v (test_0, test_1);
				(simd_type&) activeRow[k] = simd_mul_v ((simd_type&) activeRow[k], simd_andnot_v (one, simd_or_v (test_0, test_1)));

				//deltaForce[k] = accel[k] * invDJMinvJt[k] * activeRow[k];
				(simd_type&)deltaForce[k] = simd_mul_v (accel_k, simd_mul_v ((simd_type&) invDJMinvJt[k], (simd_type&) activeRow[k]));
				...
				tmp2 = simd_max_v(tmp2, simd_and_v(simd_mul_v (accel_k, (simd_type&)activeRow[k]), signMask));
			...
			tmp2 = simd_max_s (tmp2, simd_permut_v (tmp2, tmp2, PURMUT_MASK (0, 0, 0, 1)));
			...
			maxPasses = simd_store_is (simd_add_s (tmp3, simd_permut_v (tmp3, tmp3, PURMUT_MASK (0, 0, 0, 1))));
			...
			for (dgInt32 k = 0; k < roundCount; k += DG_SIMD_WORD_SIZE) {
				...
				(simd_type&)force[k] = simd_mul_add_v((simd_type&)force[k], tmp0, (simd_type&)deltaForce[k]);
				(simd_type&)accel[k] = simd_mul_sub_v((simd_type&)accel[k], tmp0, (simd_type&)deltaAccel[k]);

				//accNorm = GetMax (dgAbsf (m_accel[k] * activeRow[k]), accNorm);
				tmp2 = simd_max_v(tmp2, simd_and_v(simd_mul_v ((simd_type&)accel[k], (simd_type&)activeRow[k]), signMask));
				//_ASSERTE (dgCheckFloat(m_force[k]));
				//_ASSERTE (dgCheckFloat(m_accel[k]));

				//m_deltaForce[k] = m_accel[k] * m_invDJMinvJt[k] * activeRow[k];
				(simd_type&)deltaForce[k] = simd_mul_v((simd_type&)accel[k], simd_mul_v((simd_type&)invDJMinvJt[k], (simd_type&)activeRow[k]));

				//akNum += m_deltaForce[k] * m_accel[k];
				tmp1 = simd_mul_add_v(tmp1, (simd_type&)deltaForce[k], (simd_type&)accel[k]);
			}
			tmp1 = simd_add_v (tmp1, simd_move_hl_v(tmp1, tmp1));
			tmp1 = simd_add_s (tmp1, simd_permut_v (tmp1, tmp1, PURMUT_MASK (0, 0, 0, 1)));
			simd_store_s (tmp1, &akNum);
			...
			if (accNorm > maxAccNorm) {
				...
				for (dgInt32 k = 0; k < roundCount; k += DG_SIMD_WORD_SIZE) {
					...
					(simd_type&)deltaAccel[k] = simd_mul_v((simd_type&)accel[k], simd_mul_v((simd_type&)invDJMinvJt[k], (simd_type&)activeRow[k]));

					//akNum += m_accel[k] * m_deltaAccel[k];
					tmp0 = simd_mul_add_v(tmp0, (simd_type&)accel[k], (simd_type&)deltaAccel[k]);
				}

				tmp0 = simd_add_v (tmp0, simd_move_hl_v(tmp0, tmp0));
				tmp0 = simd_add_s (tmp0, simd_permut_v (tmp0, tmp0, PURMUT_MASK (0, 0, 0, 1)));
				simd_store_s (tmp0, &akNum);
				...
				for (dgInt32 k = 0; k < roundCount; k += DG_SIMD_WORD_SIZE) {
					...
					(simd_type&)deltaForce[k] = simd_mul_add_v((simd_type&)deltaAccel[k], tmp0, (simd_type&)deltaForce[k]);
				}
			}
		}
	}
...
void dgJacobianMemory::CalculateSimpleBodyReactionsForces (dgFloat32 maxAccNorm) const
{
		...
		for (dgInt32 k = 0; k < count; k ++) {
			if (activeRow[k]) {
				...
				if (deltaForce[k] < dgFloat32 (-1.0e-16f)) {
					if (val < lowBound[k]) {
						ak = GetMax ((lowBound[k] - force[k]) / deltaForce[k], dgFloat32 (0.0f));
						clampedForceIndex = k;
					...
					if (val >= highBound[k]) {
						ak = GetMax ((highBound[k] - force[k]) / deltaForce[k], dgFloat32 (0.0f));;
						clampedForceIndex = k;
...
void dgJacobianMemory::CalculateForcesSimulationModeSimd (dgFloat32 maxAccNorm) const
{
	...
	for (dgInt32 i = 0; i < m_jointCount; i ++) {
		...
		for (j = 0; j < count; j ++) {
			...
			y0_linear  = simd_mul_add_v (y0_linear, (simd_type&) Jt[index].m_jacobian_IM0.m_linear, tmp0);
			y0_angular = simd_mul_add_v (y0_angular,(simd_type&) Jt[index].m_jacobian_IM0.m_angular, tmp0);
			y1_linear  = simd_mul_add_v (y1_linear, (simd_type&) Jt[index].m_jacobian_IM1.m_linear, tmp0);
			y1_angular = simd_mul_add_v (y1_angular,(simd_type&) Jt[index].m_jacobian_IM1.m_angular, tmp0);
		}
...
//			dgInt32 currJoint;
			...
			for (dgInt32 i = 0; i < rowsCount; i ++) {
				...
				y0_linear  = simd_mul_add_v (y0_linear, (simd_type&) Jt[index].m_jacobian_IM0.m_linear, deltaForce);
				y0_angular = simd_mul_add_v (y0_angular,(simd_type&) Jt[index].m_jacobian_IM0.m_angular, deltaForce);
				y1_linear  = simd_mul_add_v (y1_linear, (simd_type&) Jt[index].m_jacobian_IM1.m_linear, deltaForce);
				y1_angular = simd_mul_add_v (y1_angular,(simd_type&) Jt[index].m_jacobian_IM1.m_angular, deltaForce);
...
//				acc += JMinv[index].m_jacobian_IM1.m_angular.CompProduct (y1.m_angular);
				...
				tmpAccel = simd_mul_v (              (simd_type&)JMinv[index].m_jacobian_IM0.m_linear, y0_linear);
				tmpAccel = simd_mul_add_v (tmpAccel, (simd_type&)JMinv[index].m_jacobian_IM0.m_angular, y0_angular);
				tmpAccel = simd_mul_add_v (tmpAccel, (simd_type&)JMinv[index].m_jacobian_IM1.m_linear, y1_linear);
				tmpAccel = simd_mul_add_v (tmpAccel, (simd_type&)JMinv[index].m_jacobian_IM1.m_angular, y1_angular);

//				accel[i] = coordenateAccel[index] - acc.m_x - acc.m_y - acc.m_z - force[index] * diagDamp[index];
				tmpAccel = simd_add_v (tmpAccel, simd_move_hl_v(tmpAccel, tmpAccel));
				tmpAccel = simd_add_s(tmpAccel, simd_permut_v (tmpAccel, tmpAccel, PURMUT_MASK(3, 3, 3, 1)));
				tmpAccel = simd_mul_add_s(tmpAccel, simd_load_s(force[index]), simd_load_s(diagDamp[index]));
				simd_store_s (simd_sub_s(simd_load_s(coordenateAccel[index]), tmpAccel), &accel[i]);
				index ++;
			}
	...
	for (dgInt32 i = 0; i < m_jointCount; i ++) {
		...
		for (dgInt32 k = 0; k < count; k += DG_SIMD_WORD_SIZE) {
			...
			//val = force[j];
			normalForce = simd_move_lh_v (simd_pack_lo_v (simd_load_s(force[normalForceIndex[index + 0]]), simd_load_s(force[normalForceIndex[index + 1]])), simd_pack_lo_v (simd_load_s(force[normalForceIndex[index + 2]]), simd_load_s(force[normalForceIndex[index + 3]])));

			//lowerForceBound[index] *= val;
			//upperForceBound[index] *= val;
			(simd_type&)lowerForceBound[index] = simd_mul_v (normalForce, (simd_type&)lowerForceBound[index]);
			(simd_type&)upperForceBound[index] = simd_mul_v (normalForce,  (simd_type&)upperForceBound[index]);

			//force[index] = ClampValue(force[index], lowerForceBound[index], upperForceBound[index]);
			(simd_type&) force[index] = simd_min_v ((simd_type&) upperForceBound[index], simd_max_v((simd_type&) force[index], (simd_type&)lowerForceBound[index]));
		}
	}
	...
	for (dgInt32 i = 0; i < m_jointCount; i ++) {
		...
		for (dgInt32 j = 0; j < count; j ++) {
			...
			tmp0 = simd_mul_v (          (simd_type&)JMinv[index].m_jacobian_IM0.m_linear, y0_linear);
			tmp0 = simd_mul_add_v (tmp0, (simd_type&)JMinv[index].m_jacobian_IM0.m_angular, y0_angular);
			tmp0 = simd_mul_add_v (tmp0, (simd_type&)JMinv[index].m_jacobian_IM1.m_linear, y1_linear);
			tmp0 = simd_mul_add_v (tmp0, (simd_type&)JMinv[index].m_jacobian_IM1.m_angular, y1_angular);

			//accel[index] = coordenateAccel[index] - (tmpAccel.m_x + tmpAccel.m_y + tmpAccel.m_z + force[index] * diagDamp[index]);
			tmp0 = simd_add_v (tmp0, simd_move_hl_v(tmp0, tmp0));
			tmp0 = simd_add_s(tmp0, simd_permut_v (tmp0, tmp0, PURMUT_MASK(3, 3, 3, 1)));
			tmp0 = simd_sub_s(simd_load_s(coordenateAccel[index]), simd_mul_add_s(tmp0, simd_load_s(force[index]), simd_load_s(diagDamp[index])));
	...
	for (passes = 0; (passes < maxPasses) && (accNorm > maxAccNorm); passes ++) {
		...
		for (dgInt32 i = 0; i < m_jointCount; i ++) {
			...
			for (j = 0; j < count; j ++) {
				...
				tmp2 = simd_mul_v (          (simd_type&)JMinv[index].m_jacobian_IM0.m_linear, y0_linear);
				tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[index].m_jacobian_IM0.m_angular, y0_angular);
				tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[index].m_jacobian_IM1.m_linear, y1_linear);
				tmp2 = simd_mul_add_v (tmp2, (simd_type&)JMinv[index].m_jacobian_IM1.m_angular, y1_angular);
		...
		for (dgInt32 i = 0; i < m_jointCount; i ++) {
			...
			if (simd_store_is (simd_cmpgt_s (akSimd, tol_pos_1eNeg8))) {
				...
				for (j = 0; j < count; j ++) {
					...
					test0 = simd_and_v (simd_cmplt_s (delta_force, tol_neg_1eNeg16), simd_cmplt_s (tmp1, low_force));

					//test1 = (deltaForce[index] > dgFloat32 ( 1.0e-16f)) & (tmp > upperForceBound[index]);
					test1 = simd_and_v (simd_cmpgt_s (delta_force, tol_pos_1eNeg16), simd_cmpgt_s (tmp1, high_force));

					//test =  test0 | test1;
					if (simd_store_is (simd_or_v (test0, test1))) {
						//clampedForceIndexValue = lowerForceBound[index];
						clampedForceIndexValue = simd_or_v (simd_and_v (low_force, test0), simd_and_v (high_force, test1));

						akSimd = simd_div_s (simd_sub_s (clampedForceIndexValue, force_index), delta_force);
		...
		if (clampedForceIndex >= 0) {
			...
			for (dgInt32 i = 0; i < m_jointCount; i ++) {
				...
				for (j = 0; j < count; j += DG_SIMD_WORD_SIZE) {
					...
					(simd_type&)force[index] = simd_mul_add_v((simd_type&)force[index], akSimd, (simd_type&)deltaForce[index]);
					(simd_type&)accel[index] = simd_mul_sub_v((simd_type&)accel[index], akSimd, (simd_type&)deltaAccel[index]);
				}
			}
			...
			for (dgInt32 i = 0; i < m_jointCount; i ++) {
				...
				for (j = 0; j < count; j += DG_SIMD_WORD_SIZE) {
					...
					//deltaForce[index] = accel[index] * invDJMinvJt[index];
					(simd_type&)deltaForce[index] = simd_mul_v((simd_type&)accel[index], (simd_type&)invDJMinvJt[index]);

					//akNum += deltaForce[index] * accel[index];
					tmp0 = simd_mul_add_v(tmp0, (simd_type&)accel[index], (simd_type&)deltaForce[index]);

					//accNorm = GetMax (dgAbsf (accel[index]), accNorm);
					tmp1 = simd_max_v(tmp1, simd_and_v((simd_type&)accel[index], signMask));
			...
			tmp1 = simd_max_s (tmp1, simd_permut_v (tmp1, tmp1, PURMUT_MASK (0, 0, 0, 1)));
			simd_store_s (tmp1, &accNorm);
			...
			if (accNorm > maxAccNorm) {
				...
				for (dgInt32 i = 0; i < m_jointCount; i ++) {
					...
					for (dgInt32 j = 0; j < count; j += DG_SIMD_WORD_SIZE) {
						...
						(simd_type&)deltaAccel[index] = simd_mul_v((simd_type&)accel[index], (simd_type&)invDJMinvJt[index]);

						//akNum += accel[index] * deltaAccel[index];
						tmp0 = simd_mul_add_v(tmp0, (simd_type&)accel[index], (simd_type&)deltaAccel[index]);
					}
				}
				...
				tmp0 = simd_div_s(tmp0, simd_max_s (akDenSimd, simd_set1(dgFloat32 (1.0e-17f))));
				tmp0 = simd_permut_v (tmp0, tmp0, PURMUT_MASK(0, 0, 0, 0));
				for (dgInt32 i = 0; i < m_jointCount; i ++) {
					dgInt32 first;
					dgInt32 count;
					...
					for (dgInt32 j = 0; j < count; j += DG_SIMD_WORD_SIZE) {
						...
						(simd_type&)deltaForce[index] = simd_mul_add_v((simd_type&)deltaAccel[index], tmp0, (simd_type&)deltaForce[index]);
					}
				}
			}
		}
...
void dgJacobianMemory::CalculateForcesGameModeSimd (dgInt32 iterations, dgFloat32 maxAccNorm) const
{
	...
	//				acc += m_JMinv[index].m_jacobian_IM1.m_angular.CompProduct (angularM1);
					simd_type a = simd_mul_v (       (simd_type&)JMinv[index].m_jacobian_IM0.m_linear, linearM0);
					a = simd_mul_add_v (a, (simd_type&)JMinv[index].m_jacobian_IM0.m_angular, angularM0);
					a = simd_mul_add_v (a, (simd_type&)JMinv[index].m_jacobian_IM1.m_linear, linearM1);
					a = simd_mul_add_v (a, (simd_type&)JMinv[index].m_jacobian_IM1.m_angular, angularM1);
					...
					a = simd_sub_s(simd_load_s(coordenateAccel[index]), simd_mul_add_s(a, simd_load_s(force[index]), simd_load_s(diagDamp[index])));


					//f = force[index] + invDJMinvJt[index] * a;
					simd_type f = simd_mul_add_s (simd_load_s(force[index]), simd_load_s(invDJMinvJt[index]), a);

					dgInt32 frictionIndex = m_normalForceIndex[index];
					_ASSERTE (((frictionIndex < 0) && (force[frictionIndex] == dgFloat32 (1.0f))) || ((frictionIndex >= 0) && (force[frictionIndex] >= dgFloat32 (0.0f))));
					...
					simd_type lowerFrictionForce = simd_mul_s (frictionNormal, simd_load_s(lowerFrictionCoef[index]));
					simd_type upperFrictionForce = simd_mul_s (frictionNormal, simd_load_s(upperFrictionCoef[index]));
					...
					linearM0 = simd_mul_add_v (linearM0, (simd_type&) Jt[index].m_jacobian_IM0.m_linear, a);
					angularM0 = simd_mul_add_v (angularM0,(simd_type&) Jt[index].m_jacobian_IM0.m_angular, a);
					linearM1 = simd_mul_add_v (linearM1, (simd_type&) Jt[index].m_jacobian_IM1.m_linear, a);
					angularM1 = simd_mul_add_v (angularM1,(simd_type&) Jt[index].m_jacobian_IM1.m_angular, a);
					index ++;
				}
...
void dgJacobianMemory::CalculateForcesGameMode (dgInt32 iterations, dgFloat32 maxAccNorm) const
{
	dgFloat32* const force = m_force;
	const dgJacobianPair* const Jt = m_Jt;
	const dgJacobianPair* const JMinv = m_JMinv;
...
dgInt32 dgWorldDynamicUpdate::GetJacobialDerivativesParallel (const dgIsland& island, bool bitMode, dgInt32 rowCount, dgFloat32 timestep)
{
	dgInt32 chunkSizes[DG_MAXIMUN_THREADS];
	dgJointInfo* const constraintArray = &m_constraintArray[island.m_jointStart];
...
void dgWorldDynamicUpdate::BuildJacobianMatrixParallel (const dgIsland& island, dgFloat32 timestep, dgInt32 archModel)
{
	...
	dgInt32 rowCount = 0;
	if (island.m_hasUnilateralJoints) {
		rowCount = GetJacobialDerivativesParallel (island, false, rowCount, timestep);
	}
