...
Function AddPropertyGridGroup:TPropGroup(grid:TGadget,name$)
	Return TPropGrid(grid).AddGroup(name)
End Function

Type TPropGrid Extends TProxyGadget {expose}
	...
	Field currentitem:TProp
	Field font:TGUIFont
	Field properties:TMap=New TMap

	Method Collapse()
		For Local propertygroup:TPropgroup=EachIn sections.values()
	...
	Method Combine(combiner:TPropGrid,update=1)
		Local item:TProp[2]
		Local key$
		For Local gs:TPropGroup=EachIn groups
			If Not gs.hidden
				...
				Else
					For item[0]=EachIn gs.items
						If Not item[0].hidden
							...
							item[1]=TProp(combiner.properties.valueforkey(key))
							If Not item[1]
								item[0].hide(0)
							Else
		...
		For key=EachIn properties.keys()
			item[0]=TProp(properties.valueforkey(key))
			item[1]=TProp(combiner.properties.valueforkey(key))
			If Not item[1]
				item[0].hide(0)
			Else
	...
	Method SetProperty(name:String,value:String)
		Local property:TProp
		name=name.tolower()
		property=TProp(properties.valueforkey(name))
	...
	Method Clear()
		For Local gs:TPropGroup=EachIn sections.values()
			gs.free()
		Next
	EndMethod
	
	Function Create:TPropGrid(x=0,y=0,width=-1,height=-1,group:TGadget)
		Local grid:TPropGrid=New TPropGrid
	...
	Method AddGroup:TPropGroup(name:String)
		Local gs:TPropGroup=New TPropGroup
		...
		gs.sidepanel=CreatePanel(0,0,TPropGroup.itemheight,gs.panel.height,gs.panel)
	...
	Function eventHandler:Object( pID%, pData:Object, pContext:Object )
		...
		If event
			Local grid:TPropGrid
			Local gs:TPropGroup
			Local gi:TProp
			
			For grid=EachIn TPropGrid.list
...
Type TPropGroup {expose}

	Const orglabelwidth:Int=100
	...
	Field labelwidth=orglabelwidth
	
	Field link:TLink
	Field items:TList=New TList
	Field grid:TPropGrid
	...
	Method ShowAll()
		show(0)
		For Local item:TProp=EachIn items
			item.show(0)
		Next
	EndMethod
	...
	Method AdjustLayout()
		...
		For Local item:TProp=EachIn items
			...
			Else
				SetGadgetShape item.label,GadgetX(item.label),y+2,GadgetWidth(item.label),GadgetHeight(item.label)
	...
	Method Expand(update=1)
		collapsed=False
		'For Local gs:TPropGroup=EachIn grid.groups
	...
	Method AddProperty:TProp(name:String,value:String="",style:Int=0,tooltip:String="",flags:Int=0,label:String="")
		Local griditem:TProp=New TProp
...
Type TProp
	...
	Method Compare:Int(o:Object)
		Return GadgetText(label).compare(GadgetText(TProp(o).label))
	EndMethod
	...
	Method EventHook:TEvent( event:TEvent,grid:TPropGrid )
		Local s:String
